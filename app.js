...
const sortedByBirthYear = inventors.toSorted(
...
const totalYearsLived = inventors.reduce((total, current) => {
  const inventorYears = current.passed - current.year;
  return total + inventorYears;
}, 0);
...
const travelMethodCounts = travelMethods.reduce((acc, current) => {
  if (current in acc) {
    acc[current] += 1;
  } else {
    acc[current] = 1;
